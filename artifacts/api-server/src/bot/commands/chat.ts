...
const ASTRAL_RESPONSES: Record<string, string[]> = {
  ...
  aide: ["Mes capacités incluent : modération avancée, sécurité, giveaways, questionnaires. Utilisez `/commands` pour la liste complète.", "Je peux vous aider avec la gestion du serveur, les systèmes de sécurité, et bien plus. Tapez `/commands`."],
...
const DEFAULT_RESPONSES = [
  ...
  "Intéressant. Mes algorithmes ne trouvent pas de réponse directe. Essayez `/commands` pour explorer mes capacités.",
  ...
  "Analyse linguistique terminée. Recommandation : utilisez `/commands` pour voir ce que je peux faire.",
];
...
export const chatCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const embed = new EmbedBuilder()
      .setTitle("🤖 ASTRAL-BOT — Intelligence Artificielle")
      .setColor(0x5865f2)
      .addFields(
        { name: "👤 Vous", value: message, inline: false },
        { name: "🤖 ASTRAL-BOT", value: response, inline: false },
      )
      .setFooter({ text: "ASTRAL TECHNOLOGIE | IA v2.0" })
      .setTimestamp();
